...
setup(
    ...
    entry_points={
        "console_scripts": [
            "epistemological-validation=epistemological-validation.cli:main",
        ],
    },
)
```
...
cd AI-Consciousness-Sensors/epistemological-validation
pip install -e .
```
...
```python
from epistemological-validation import MultiEpistemologicalValidator, ConsciousnessClaimData
...
```python
from epistemological-validation.mcp_integration import EpistemologicalValidationServer
...
def test_funding_bias-detection():
...
database:
  path: "epistemological-validation.db"
